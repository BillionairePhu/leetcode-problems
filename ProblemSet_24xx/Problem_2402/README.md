...
> **Problem:** [Meeting Rooms III](https://leetcode.com/problems/meeting-rooms-iii)<br />
> **Difficulty:** Hard<br/>
...
Just sort the room by its starting time and then process it by:

- If there is an empty room (room with available time less than or equal to the current meeting's starting time), assign the first room we encounter (by incrementing `meeting_count` for that room and updating the earliest available time of it).

- If not, find the room with the earliest available time and assign that room with the current meeting.
