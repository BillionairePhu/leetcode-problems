...
> **Problem:** [Divide an Array Into Subarrays With Minimum Cost II](https://leetcode.com/problems/divide-an-array-into-subarrays-with-minimum-cost-ii)<br />
> **Difficulty:** Hard<br/>
...
The key point of this problem is that for each `i` starting from `1` to `len(nums)-1`, we need to find `k-2` smallest number to be the starting point of the sub-arrays. We can do this efficiently using a binary tree (SortedList in Python) and a sliding window.
