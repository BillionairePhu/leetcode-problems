...
> **Problem:** [Minimum Cost to Convert String I](https://leetcode.com/problems/minimum-cost-to-convert-string-i)<br />
...
Given $\{K_1,K_2,\dots\}$ are the nodes and $C_{i,j}\in[0,\infty]$ is the cost the travel from node.

Let $C_{i,j}(n)\in\R^{N\times N}$ be the minimum costs of travel from node $K_i$ to $K_j$ with a path only containing nodes in the set $\{K_1,K_2,\dots, K_n\}\cup\{K_i,K_j\}$. 

Then $C_{i,j}(n+1)=\min(C_{i,j}(n),C_{i,n+1}(n)+C_{n+1,j}(n))$

Hence we can calculate all $C_{i,j}(n+1)$ by calculating $C_{i,j}(k)$ for all $i,j$ for $k$ range from $0$ to $1$.

The complexity of the algorithm is therefore $N^3$, and since we are only using lower case characters, $26^3$.
