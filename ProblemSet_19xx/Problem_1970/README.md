...
> **Problem:** [Last Day Where You Can Still Cross](https://leetcode.com/problems/last-day-where-you-can-still-cross)<br />
> **Difficulty:** Hard<br/>
...
We will do a binary search with the lower bound for the answer being 0 and the upper bound being the number of cells in the matrix.

For each binary search iteration, we will check if at that day, we can still travel. To check, we will do depth-first-search for all "land" cell at the first row and expand outward. The check will return true if we can reach the bottom row.

---

### 🧪 Complexity

The binary search step in general takes O(log(m * n)) and the check for travelling takes O(m*n)
