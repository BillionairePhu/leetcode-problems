# 🔥 Leetcode Problem (3453)

> **Problem:** [Separate Squares I](https://leetcode.com/problems/separate-squares-i)<br />
...
This problem will use binary search. We will have a upper limit and a lower limit, then continously halve the range by lower top or increasing low based on whether the area under `mid` (sum average of top and low) is more or less than half of the total sum. => We do this until we reach a point where mid is (to a precision) exactly half of the total sum.

BUT we will still need to lower the `top` if the result is exactly of the total sum because we want the result to be minimum!!! Remember that there exist cases when multiple values can divide the squares into equal sum.
