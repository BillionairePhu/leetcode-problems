...
> **Problem:** [Minimum Removals to Balance Array](https://leetcode.com/problems/minimum-removals-to-balance-array)<br />
...
Instead of removing the minimum amount of elements, think of keeping an array `subarr` from the array that satisfies `max(subarr) <= k * min(subarr)` and maximizing the count of it.

If we first sort the `nums` then the problem simplify into a 2-pointer problem.
