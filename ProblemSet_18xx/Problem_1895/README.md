...
> **Problem:** [Largest Magic Square](https://leetcode.com/problems/largest-magic-square)<br />
...
The largest possible size of a magic square in a given grid of size `m x n` would be the smaller of the grid's 2 sizes. From that largest possible size, we decrement and check if we can find any magic squares of that size.

To make the checking of lower complexity, we will use prefix sums for all: rows, columns, up-diagons and down-diagons.

---

### 🧪 Complexity

- **Time:** `O(m * n * min(m*n) * (m + n))`
- **Space:** `O(m * n)`